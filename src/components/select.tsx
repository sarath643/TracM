  ...
  return (
    <div>
      ...
      <Listbox value={value} onChange={onChange}>
        <ListboxButton
          className={clsx(
            'relative block h-10 dark:border-gray-700  dark:border rounded-lg capitalize bg-gray-200  dark:bg-gray-900  py-1.5 pr-8 pl-3 text-left text-sm/6 text-blackT dark:text-white',
        ...
        <ListboxOptions
          ...
          {options.map((item) => (
            <ListboxOption
              key={item.label}
              value={item}
              className='group flex h-10 cursor-default items-center gap-2 rounded-lg py-1.5 px-3 select-none data-[focus]:bg-gray-300 dark:data-[focus]:bg-white/10'>
